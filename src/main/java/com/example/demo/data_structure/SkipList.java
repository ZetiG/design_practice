...
public class SkipList<T> {
    ...
    public void add(Node<T> node) {
        ...
        while (head != null) {
            ...
            // 右侧无节点 或者 右侧节点大于当前插入节点，向下
            if (head.right == null || head.right.idx > idx) {

                stack.add(head);//将曾经向下的节点记录一下
                head = head.down;
                continue;
            ...
            while (!stack.isEmpty()) {
                ...
                //在该层插入node
                head = stack.pop();//抛出待插入的左侧节点
                Node nodeTeam = new Node(node.idx, node.val);//节点需要重新创建
                nodeTeam.down = downNode;//处理竖方向
                downNode = nodeTeam;//标记新的节点下次使用
                if (head.right == null) {//右侧为null 说明插入在末尾
                    head.right = nodeTeam;
                }
                //水平方向处理
                else {//右侧还有节点，插入在两者之间
                ...
                if (level > MAX_LEVEL)//已经到达最高级的节点啦
                    break;
                double num = RandomUtils.nextDouble();//[0-1]随机数
                if (num > 0.5)//运气不好结束
                    break;
                level++;
                if (level > highLevel)//比当前最大高度要高但是依然在允许范围内 需要改变head节点
                {
                    highLevel = level;
                    //需要创建一个新的节点
                    Node highHeadNode = new Node(Integer.MIN_VALUE, null);
    ...
    public void printList() {
        ...
        while (teamNode!=null) {
            ...
            while (enumLast!=null&&enumNode!=null) {
                ...
                {
                    System.out.printf("%-5s",enumLast.idx+"->");
